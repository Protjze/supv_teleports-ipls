...
for k,v in pairs(teleports) do
    options[#options+1] = {
        title = ("[%s] - %s"):format(k, v.text),
        description = ('[%s] Vehicle authorised? : %s'):format(k,not v.veh and 'True' or 'False'),
        onSelect = function()
            SetEntityCoords(cache.ped, v.coord.x, v.coord.y, v.coord.z)
            SetEntityHeading(cache.ped, v.h)
            print(("Vous êtes actuellement allé:\n\t- key : %s\n\t- text: %s"):format(k, v.text))
        end
    }
end
...
lib.registerContext({
    id = 'teleport:tool',
    title = "Teleport Ipl's",
    options = options
})

RegisterCommand('tl', function()
    if not lib.getOpenContextMenu() then
        lib.showContext('teleport:tool')
    elseif lib.getOpenContextMenu() ~= 'teleport:tool' then
        lib.hideContext()
        Wait(100)
        lib.showContext('teleport:tool')
    else
